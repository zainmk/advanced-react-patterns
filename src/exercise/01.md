...
- Context Module Function -> When using the context, expose the context provider, use hook, and any helper functions relevant to each dispatch type. All to be imported in separately.
- Typically keep all context code within a single file
- Adds another layer to the API so dispatch functions are provided (instead of needing to be referenced). Not by individually wrapping each dispatch type in its own method, but by exposing as an export.
- Context's can be given 'display names' to identify in React DevTools
- Similar to creating a 'setter' for your context API (tied to the dispatch methods)
