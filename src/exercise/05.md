...
- State Reducer Pattern inverts the control to the programmer opening the API for the component. 
- Opening up the reducer, while providing a default, allows the programmer to write their own code for each dispatch action type.
- Reducer provided and passed through the hook. 
- If providing a default template for the reducer, however a reference to one to expose the API to changes, be sure to export the default redeucer as well with the useHook.
- We can export and type constraint the possible action types for the reducer, instead of managing the hardcoded strings. Easier API to use.
