...
- Accessibility for custom components can be required, especially for UI components.
- Exposing the props through the hook allows users to override them if required and pass them through to the HTML elements.
- Can also set default props when passed, like onClick's or classNames, etc...
- Using a method (prop getter) to organize and deliver the props, allows more control over any additional props and checking (ex. creating an API to allow onClick to be overridden). Further expands the API.
