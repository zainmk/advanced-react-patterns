...
- Compound Components: A family of components, held by a parent, to share state and an effective API for reusability. State is shared inherently without having to pass as props.
- React.Children.map() => outputs the children with reference to the current component
- React.cloneElement(element, props) => returns a copy of the react element however with the additional props.
- To imperatively pass props through to the children, map out the children elements, cloning and returning each with the additional fed props. Otherwise React does not allow manipulation of element props specifically. This allows us to implicity pass props through the children components and share state amongst the component family
- Cloning is not performance heavy (as React compiles code before it creates the DOM elements)
- Beware of HTML elements within the component family, and passing props through to them as well (where props are not valid). Type check within the parent component, before returning copy of child (or just return the child).
