...
- In the case of compound components, nested children DO NOT end up being copied and returned with new props. (With the children mapping and cloning)
- Use React.Context to share state inherently within the React family tree.
- Return the provider through the parent component (so its wrapped around all children element that require access - keeps context tight)
- Perhaps consider keeping the context within the parent component that holds the entire family. Keep context logic close to the context the parent component provides.
- Add error boundaries to ensure child element is not used without parent element context (if child MUST be cared for by parent).
- Context.displayName = '', to uniquely identify in ReactDevTools
