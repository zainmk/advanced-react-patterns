...
- Control Props pattern allows full control of the state of the components, allowing external state changes, and not just those through the possible actions of the provided reducer.
- State changes can stay synchronized between the components (as the state itself can now be manipulated)
- When exposing the dispatch through the props, check for controlled components, to limit unnecessary dispatch calls and rerenders
- Be aware of which props may be passed and which may not (if no onChange is provided, ensure a default is called)
- Consider putting warnings in Hooks to reuse. Any reusable code can be put in a hook.
- Note that uncontrolled components throw warnings once the value is in a weird state that it cannot register. (Error check the 'controlled' variable value)
